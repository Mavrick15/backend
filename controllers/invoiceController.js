...
const LOG_MESSAGES = {
  INFO_INVOICE_CREATION_START: (userName) =>
    `Création de facture pour l'utilisateur: ${userName}`,
  INFO_INVOICE_CREATED: (invoiceNumber, userName) =>
    `Facture ${invoiceNumber} créée pour ${userName}`,
  WARN_FORMATION_NOT_FOUND: (formationId) =>
    `Formation non trouvée: ${formationId}`,
  ERROR_INVOICE_CREATION: "Erreur lors de la création de la facture:",
  INFO_INVOICES_FETCHED: (userId) =>
    `Factures récupérées pour l'utilisateur: ${userId}`,
  INFO_INVOICE_FETCHED: (invoiceNumber) =>
    `Facture récupérée: ${invoiceNumber}`,
...
const getUserInvoices = async (req, res, next) => {
  try {
    ...
    logger.info(LOG_MESSAGES.INFO_INVOICES_FETCHED(userId), {
    ...
    logger.error(LOG_MESSAGES.ERROR_INVOICE_FETCH, {
      message: error.message,
      stack: error.stack,
    });
    next(error);
...
const getInvoiceById = async (req, res, next) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        .status(400)
        .json({ success: false, message: "ID de facture invalide" });
    }
    ...
    if (!invoice) {
      ...
      return res
        .status(404)
        .json({ success: false, message: MESSAGES.INVOICE_NOT_FOUND });
    }

    // Vérifier que l'utilisateur est propriétaire de la facture ou admin
    if (
      invoice.user._id.toString() !== userId.toString() &&
      req.user.role !== "admin"
    ) {
      return res
        .status(403)
...
const updateInvoiceStatus = async (req, res, next) => {
  try {
    ...
    if (
      invoice.user.toString() !== userId.toString() &&
      req.user.role !== "admin"
    ) {
      return res
        .status(403)
    ...
    logger.error("Erreur lors de la mise à jour de la facture:", {
      message: error.message,
      stack: error.stack,
    });
    next(error);
